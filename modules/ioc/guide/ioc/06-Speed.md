...
XML definition file created in the previous chapter:
    ...
    $dumper = new Service_Container_Dumper_Php($sc);

    $code = $dumper->dump(array('class' => 'Container'));

    file_put_contents('/somewhere/container.php', $code);

As for any other dumper, the [Service_Container_Dumper_Php] class takes a
...
By using the [Service_Container_Dumper_Php] dumper, you can have the best of
...
mode:
    ...
    [php]
    $name = 'Project'.md5($appDir.$isDebug.$environment).'Service_Container';
...
First, let's see how to use it on our example container:
    ...
    [php]
    $dumper = new Service_Container_Dumper_Graphviz($sc);
...
[!!]The Graphviz dumper `dump()` method takes a lot of different options to tweak the output of 
the graph.

Have a look at the source code to discover the default values for each of them:  
