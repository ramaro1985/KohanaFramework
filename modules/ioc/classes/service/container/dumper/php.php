...
/**
 * Service_Container_Dumper_Php dumps a service container as a PHP class.
 *
 * @package    Kohana/IOC
 ...
 * @version    SVN: $Id$
 */
class Service_Container_Dumper_Php extends Service_Container_Dumper
{
  /**
...
EOF;
  ...
  protected function addServiceConfigurator($id, $definition)
  {
    ...
    {
      if (is_object($callable[0]) && $callable[0] instanceof Service_Reference)
      {
  ...
  {
    $name = Service_Container::camelize($id);
...
EOF;
  ...
  {
    $name = Service_Container::camelize($alias);
...
EOF;
  ...
  protected function exportParameters($parameters, $indent = 6)
  {
    ...
    foreach ($parameters as $key => $value)
    {
      ...
      elseif ($value instanceof Service_Reference)
      {
        $value = sprintf("new Service_Reference('%s')", $value);
      }
      elseif ($value instanceof Service_Parameter)
      {
...
EOF;
  ...
  protected function dumpValue($value)
  {
    ...
    elseif (is_object($value) && $value instanceof Service_Reference)
    {
      return $this->getServiceCall((string) $value);
    }
    elseif (is_object($value) && $value instanceof Service_Parameter)
    {
