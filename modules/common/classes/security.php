...
class Security extends Kohana_Security 
{
     ...
     * @param   bool
     * @uses    Text::remove_invisible_characters
     * @return  string
     */
    public static function xss_clean($str, $is_image = FALSE)
    {
         ...
         */
        $str = Text::remove_invisible_characters($str);
        ...
        $str = preg_replace_callback("/[a-z]+=([\'\"]).*?\\1/si", 'self::_convert_attribute', $str);
        ...
        foreach ($words as $word)
        {
            ...
            $str = preg_replace_callback('#('.substr($temp, 0, -3).')(\W)#is', 'self::_compact_exploded_words', $str);
        }
        ...
        do
        {
            ...
            {
                $str = preg_replace_callback("#<a\s+([^>]*?)(>|$)#si", 'self::_js_link_removal', $str);
            }

            if (preg_match("/<img/i", $str))
            {
                $str = preg_replace_callback("#<img\s+([^>]*?)(\s?/?>|$)#si", 'self::_js_img_removal', $str);
            }
	...
	{
		return html_entity_decode($match[0], strtoupper(Kohana::$charset));
	}
    ...
    protected static function _validate_entities($str)
    {
        ...
        $str = preg_replace('|\&([a-z\_0-9\-]+)\=([a-z\_0-9\-]+)|i', aelf::xss_hash()."\\1=\\2", $str);
