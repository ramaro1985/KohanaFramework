...
Extop.Preferences.CheckTree = Ext.extend(Ext.tree.TreePanel, {
   /**
    * @cfg {String}
    */
   launcherKey: null,
   ...
   constructor : function(config){
      ...
      this.launcherKey = config.launcherKey || null;
		...
		Extop.Preferences.CheckTree.superclass.constructor.apply(this, [config]);
		...
		function expandNodes(ms, ids){
			var nodes = [];
                        var mspriv = config.ownerModule.app.privileges.modules;
			for(var i = 0, len = ms.length; i < len; i++)
                        {
                             if(mspriv.indexOf(ms[i].id.toLowerCase()) !== -1)
                             {
