...
Ext.Desktop = Ext.extend(Ext.util.Observable, {
    ...
    constructor : function(config){
        ...
        this.contextMenu = new Ext.menu.Menu({
            items: [
                {
                     text: this.app.locale.extop.close_all,
                     handler: this.closeWindows,
                     scope: this
                },
                '-',
                {
                    text: this.app.locale.extop.minimize_all,
                    handler: this.minimizeWindows,
                    scope: this
                },
                '-',
                {
                     text: this.app.locale.extop.cascade,
                     handler: this.cascadeWindows,
                     iconCls: 'application_cascade-icon',
                     scope: this
                },
                {
                     text: this.app.locale.extop.tile,
                     handler: this.tileWindows,
                     iconCls: 'application_view_tile-icon',
                     scope: this
                },
                {
                     text: this.app.locale.extop.checkerboard,
                     handler: this.checkerboardWindows,
                     iconCls: 'application_view_tile-icon',
                     scope: this
                },
                {
                     text: this.app.locale.extop.snap_fit,
                     handler: this.snapFitWindows,
                     iconCls: 'application_tile_horizontal-icon',
                     scope: this
                },
                {
                     text: this.app.locale.extop.snap_fit_vertical,
                     handler: this.snapFitVWindows,
                     iconCls: 'application_tile_vertical-icon',
                     scope: this
                },
                '-'
            ]
     ...
     */
     setTheme : function(o){
          if(o && o.id && o.name && o.file){
                ...
                if(o.name == "Blue"){
                	Ext.util.CSS.swapStyleSheet('theme-addon', '/modules/extop/resources/x-blue.css');
                } else if (o.name == "Gray"){
                	Ext.util.CSS.swapStyleSheet('theme-addon', '/modules/extop/resources/x-gray.css');
                } else if (o.name == "Access"){
                     Ext.util.CSS.swapStyleSheet('theme-addon', '/modules/extop/resources/x-access.css');
                }
    ...
    setWallpaper : function(o){
        if(o && o.id && o.name && o.file){
            ...
            var notifyWin = this.showNotification({
                html: this.app.locale.extop.loading_wallpaper,
                title: this.app.locale.extop.please_wait
            });
        ...
        function verify(){
            if(wp.complete){
                ...
                notifyWin.setTitle(this.app.locale.common.finished);
                notifyWin.setMessage(this.app.locale.extop.loaded_wallpaper);
	...
	buildStartItemConfig : function(){
		...
		var sortFn = this.startMenuSortFn;
                var module = this.app.privileges.modules;
                if(ms)
                {
                    var launcherPaths;
                        var paths;
                        ...
                        { 
                         if(module.indexOf(ms[i].id.toLowerCase()) !== -1)
