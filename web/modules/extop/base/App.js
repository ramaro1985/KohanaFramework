...
Ext.extend(Ext.app.App, Ext.util.Observable, {
    ...
    loadModule : function(m){
        if(m.isLoading){ return false; }
     	...
     	var notifyWin = this.desktop.showNotification({
			html: this.locale.common.loading + ' ' + moduleName + '...'
			, title: this.locale.extop.please_wait
		});
		...
		m.isLoading = true;
     	...
     	Ext.Ajax.request({
     		...
     		success: function(o){
				...
				notifyWin.setTitle(this.locale.common.finished);
				notifyWin.setMessage(moduleName + ' ' + this.locale.common.loaded + '.');
				...
				notifyWin = null;
     			...
     			if(o.responseText !== ''){
     				...
     				//Load css files associated to module
                          var cssFiles = response.css;
     				var i = this.cssLoadedFiles.length + 1;
                          
                          
     				Ext.each(
                                cssFiles, 
                                function(cssFile){
                                     if(this.cssLoadedFiles.indexOf(cssFile) == -1){
                                          this.cssLoadedFiles.push(cssFile);
                                          Ext.util.CSS.swapStyleSheet('css-' + i, cssFile);
                                          i++;
                                     }
                                },
                                this
                          ...
                          var jsFiles = response.javascript;
                          var jsFilesToLoad = new Array();
                          
                          Ext.each(
                                jsFiles, 
                                function(jsFile){
                                     if(this.jsLoadedFiles.indexOf(jsFile) == -1){
                                          this.jsLoadedFiles.push(jsFile);
						          jsFilesToLoad.push(jsFile);
                                     }
                                },
                                this
                          );
     				...
     				Ext.Msg.alert(this.locale.common.error, this.locale.extop.server_error);
     			}
     		},
     		failure: function(){
     			Ext.Msg.alert(this.locale.common.error, this.locale.extop.connection_failed);
    ...
    /**
     * @param {string} controller The controller without Controller_ preffix
     * @param {string} id The action without action_preffix
     */
    isAllowedTo : function(controller, action){
        controller = controller.toLowerCase();
        action = action.toLowerCase();
        if(controller !== '' && action != ''){
            var p = this.privileges.controller;
            for(var i = 0; i < p.length; i++)
            {
               if(p[i][controller] && Ext.isArray(p[i][controller]) && p[i][controller].indexOf(action) !== -1)
               {
                return true;
               }  
            }
    ...
    hasRole: function (role){
        ...
        Ext.each(this.memberInfo.roles, 
            function(role_info){
                if (role_info.role_id == role || role_info.name == role)
                {            
                    isTrue = true;
                    return;
                }
